...
GAP_EXAMPLES = [
    ...
    GapExample(
        ...
        gap_description=(
            ...
            "standard access within the system. "
    ...
    GapExample(
        ...
        gap_status="informal process",
        gap_title="Absence of Formal Terminated Access Removal Process",
        gap_description=(
    ...
    GapExample(
        control_id="APD-02",  # Assuming APD-02 based on termination context; adjust if needed
        gap_status="gap",
        gap_title="Absence of Terminated Access Removal Process",
        gap_description=(
            "The organization has no established process for removing system access upon "
            "employee termination. "
            "There is inconsistency in notifying system administrators when employees leave the organization, "
    ...
    GapExample(
        control_id="APD-03",
        gap_status="gap",
        gap_title="Absence of Periodic Roles Permissions Reviews",
        gap_description=(
            "Management is not currently performing a consistent periodic review of all actively held roles and their permissions "
            "within the system."
            "Since the System allows for role and permission customization, the absence of a periodic review presents a risk, as "
            "modifications to existing roles could result in innapropriate access."
            "Without regular reviews, it can be challenging to identify potential segregation of duties "
            "conflicts or excessive permissions that may exist within the roles being used in the System."
        ),
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Establish a formal periodic roles permissions review process, assigning clear ownership to qualified personnel. We recommend performing these reviews annually; however, a more frequent review (e.g., semi-annually) may be appropriate for high-risk roles or systems undergoing frequent role changes. "
            ...
            "5. Implement management sign-off to certify the periodic review completion and remediation. "
    ...
    GapExample(
        control_id="APD-03",
        gap_status="informal process",
        gap_title="Informal Periodic Roles Permissions Review Process",
        gap_description=(
            "Management conducts occasional reviews of the System roles and permissions, but lacks a "
            "formalized, documented periodic review process with consistent scheduling and methodology. "
            ...
            "the completeness and accuracy of reviews. "
            "Without a more structured approach, the assessment of segregation of duties risks "
            "and potential excessive permissions within the System may be inconsistent."
        ),
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Establish a formal periodic roles permissions review process, assigning clear ownership to qualified personnel. We recommend performing these reviews annually; however, a more frequent review (e.g., semi-annually) may be appropriate for high-risk roles or systems undergoing frequent rolechanges. "
            "2. Create review templates for documenting roles permissions reviews and for evidencing completion each review period. "
            "3. Maintain a complete inventory of roles within the application, including standard and custom roles. "
            "4. Document role purposes, permissions, and user assignments to assess appropriateness and segregation of duties. "
            "5. Implement management sign-off to certify the periodic review completion and remediation. "
    ...
    GapExample(
        control_id="APD-04",
        gap_status="gap",
        gap_title="Absence of Periodic User Access Reviews",
        gap_description=(
            "Management does not currently perform periodic user access reviews to validate the appropriateness of access "
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Establish a formal user access review process with clear definitions of management ownership and responsibilities. This access reviews should default to quarterly but can be formally adjusted depending on the nature of the system (e.g., management can consider semi-annual reviews for lower risk systems like IT tools). "
            ...
            "3. Generate user access listings, including generic and non-human accounts, in the last few days of each review period, including all active users, their roles / permissions, and if possible, their account creation and last login dates. "
            "4. Implement formal IPE procedures involving the documentation of report generation / query evidence. "
            "5. Ensure that reviews are performed and any action items taken within 30 days of period end. These reviews include secondary reviews to independently evaluate the primary reviewer's access. "
            "6. Formalize remediation / lookback procedures and define timeframes for the implementation of access changes. "
            "7. For any users noted as having inappropriate access, the reviewer should state when the access became inappropriate. A comparison of the user's last login date (where possible to obtain) to this date should be performed to determine whether the user logged in after the date access became inappropriate. If the user logged in, management should examine audit trails or system generated reports to determine what actions the user took and whether remediation is needed. "
            "8. For any access changes noted, retain evidence that the actions were taken as part of the review and perform final sign-off after all modifications noted by the reviewer(s) were taken."
        )
    ),
    ...
    GapExample(
        control_id="APD-04",
        gap_status="informal process",
        gap_title="Informal Periodic User Access Reviews",
        gap_description=(
            "Management performs occasional reviews of user access, but lacks a formalized, "
            "documented periodic review process with consistent scheduling and methodology. "
    ...
    # Generic/Non-Human Accounts Management
    GapExample(
        control_id="APD-05",
        gap_status="gap",
        gap_title="Absence of Formalized Controls for Generic/Non-Human Account Management",
        gap_description=(
        ...
        recommendation=(
            ...
            "4. Periodically review generic accounts, confirming adherence to procedures and continued business justification. This review can often be incorporated into the periodic user access review process (see APD-04). "
            "5. Train personnel on credential management policies and secure handling practices."
        )
    ),
    
    # Generic/Non-Human Accounts Management - Informal Process
    GapExample(
        control_id="APD-05",
        gap_status="informal process",
        gap_title="Informal Controls for Generic/Non-Human Account Management",
        gap_description=(
            "Management has established generic/non-human accounts within the system with some informal "
    ...
    GapExample(
        control_id="APD-06",
        gap_status="gap",
        gap_title="Absence of Periodic Admin Activity Log Reviews",
        gap_description=(
            "Management does not currently perform periodic reviews of administrator activity logs within the system, "
            ...
            "activities such as direct data changes, transactions, and other privileged actions performed within the system. "
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Establish a formal process for the periodic review of the system administrator activity logs. We recommend performing these reviews monthly; however, the frequency can be adjusted (e.g., quarterly) based on risk factors such as the volume of administrator activity or system criticality. "
            "2. Create review templates to document these periodic admin activity reviews, including assessments of inappropriate actions. "
    ...
    GapExample(
        control_id="APD-06",
        gap_status="informal process",
        gap_title="Informal Periodic Admin Activity Log Reviews",
        gap_description=(
            "Management occasionally reviews administrator activity logs within the system on an ad-hoc basis, "
    ...
    GapExample(
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Create a formal Company IT Authentication Policy document that addresses password setting standards for SOX applications that will be reviewed and approved annually. "
            "2. Establish a formal process for the annual review of password settings, including the creation of a review template to document the review. "
            "3. For systems not subject to single sign-on, review specific settings including: minimum character length, complexity requirements, lockout policies, password history, and multi-factor authentication settings. "
            "4 . For systems subject to single sign-on, review specific settings including: minimum character length, complexity requirements, lockout policies, password history, and multi-factor authentication settings. Additionally, include configuration screenshots showing the applications subject to single sign-on. "
            "5. Document system screenshots of all current authentication configurations. All screenshots should show the date / time and any email communication with vendors to obtain settings should be included in the review documentation. "
            "6. Define procedures for the handling of settings that don't adhere to the authentication policy but cannot be modified by Company as part of the security policy."
        )
    ),
    ...
    GapExample(
        control_id="CM-01",
        gap_status="gap",
        gap_title="Absence of Formal Change Management Processes",
        gap_description=(
            "Management lacks a formal, documented change management process for financially relevant system changes, "
            "leading to primarily ad-hoc modifications without consistent oversight, testing, or approval. "
            ...
            "Financially relevant changes are sometimes made directly in the production environment without prior testing in a "
            "non-production environment, increasing the risk of unintended consequences that could impact financial data integrity. "
            ...
            "Segregation of duties is not consistently enforced between developing, testing, approving, and deploying changes."
        ),
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Develop a formal change management policy outlining the process for requesting, approving, testing, and deploying financially relevant system changes. "
            "2. Implement a standardized change request process using a ticketing system to formally document all proposed changes, approvals, and testing evidence. "
            "3. Where possible, establish and utilize a non-production environment for testing system changes prior to deployment in production. "
            "4. Define appropriate testing requirements and standards to validate that changes function as intended before deployment. "
            "5. Implement a formal approval workflow requiring documented sign-off from relevant stakeholders (e.g., business owner, IT management) at key stages (request, testing completion, pre-deployment). "
            "6. Ensure segregation of duties by preventing individuals from developing, testing, approving, and deploying their own changes without independent review and authorization. Ideally, deployment to production should be performed by someone other than the developer. "
            "7. Maintain comprehensive documentation of all change requests, test results, approvals, and implementations for audit purposes."
        )
    ),
    ...
    GapExample(
        control_id="CM-01",
        gap_status="informal process",
        gap_title="Informal Change Management Processes",
        gap_description=(
            "Management follows an informal, often undocumented change management process for financially relevant system changes, "
            "relying primarily on email communications and verbal approvals rather than a structured workflow. "
            "While some change requests may be documented, there is no standardized process or centralized "
            "tracking system to ensure consistency and completeness in how changes are requested, approved, tested, "
            "and implemented. "
            "Limited testing of changes may occur, but it is conducted inconsistently and without formal "
            "testing procedures, documentation standards, or consistent use of a non-production environment to fully validate changes. "
            "Approval for changes is often obtained informally, sometimes through email or verbal confirmation, but "
            "lacks a structured workflow with clearly defined roles, responsibilities, and formal sign-offs before production deployment. "
            "Segregation of duties between developing, testing, approving, and deploying changes is not formally enforced. "
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Formalize the existing change management approach into a documented policy covering financially relevant system changes. "
            "2. Implement a standardized change request process using a ticketing system to formally document all changes, approvals, and testing evidence. "
            "3. Consistently utilize a non-production environment (where available) for testing system changes prior to deployment in production. "
            "4. Define and enforce appropriate testing requirements and standards to validate that changes function as intended before deployment. "
            "5. Implement a formal approval workflow requiring documented sign-off from relevant stakeholders at key stages (request, testing completion, pre-deployment). "
            "6. Formally define and enforce segregation of duties, preventing individuals from developing, testing, approving, and deploying their own changes without independent review. Ensure final deployment approval/migration is handled separately from development where possible. "
            "7. Standardize and maintain comprehensive documentation of all change requests, test results, approvals, and implementations for audit purposes."
        )
    ),
    ...
    GapExample(
        control_id="CM-02",
        gap_status="gap",
        gap_title="Absence of Separate Environment",
        gap_description=(
            "Management does not currently have a separate pre-production environment established for the system, "
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Collaborate with the vendor to establish a separate pre-production environment for testing purposes. "
    ...
    GapExample(
        control_id="CM-02",
        gap_status="informal process",
        gap_title="Underutilized Separate Environment",
        gap_description=(
            "Management has established a separate pre-production environment for the system, but it is not "
    ...
    GapExample(
        control_id="CM-03",
        gap_status="gap",
        gap_title="Absence of Periodic Change Review Process",
        gap_description=(
            "Management does not currently perform periodic reviews of changes deployed to the system's production "
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Establish a formal periodic review process to validate all changes deployed to the system's production environment. We recommend performing this review monthly; however, the frequency can be adjusted (e.g., quarterly) based on the volume and criticality of changes. "
            "2. Create review templates to document these periodic change reviews, including assessments of unauthorized changes. "
            "3. Determine which change types should be monitored (the following should be included for the system: configuration, workflow, integration, bundle audit trail, and script changes). "
    ...
    GapExample(
        control_id="CM-03",
        gap_status="informal process",
        gap_title="Informal Periodic Change Review Process",
        gap_description=(
            "Management occasionally reviews changes deployed to the system's production environment, but "
            "lacks a formalized, documented periodic review process with consistent scheduling and methodology. "
    ...
    GapExample(
        control_id="CM-04",
        gap_status="gap",
        gap_title="Absence of Vendor Change Management Process",
        gap_description=(
            "Management has not established a formal process to document, test, and approve critical releases, "
            "patches, or upgrades from the vendor (SaaS provider) before they are implemented in the production "
            "environment. "
            "There isn't a ticketing system or other formal documentation method used to track vendor-initiated changes "
            "to the system, resulting in reduced visibility into system modifications. "
        ...
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Develop a formal vendor change management policy specific to system updates, patches, and releases. "
            ...
            "3. Request a sandbox or test environment from the vendor to evaluate updates prior to migration to the production environment. "
    ...
    GapExample(
        control_id="CM-04",
        gap_status="informal process",
        gap_title="Informal Vendor Change Management Process",
        gap_description=(
            "Management has an informal approach to handling system (SaaS provider) updates, patches, and releases, "
    ...
    GapExample(
        control_id="MO-01",
        gap_status="gap",
        gap_title="Absence of Scheduled Job / Integration Failure Monitoring",
        gap_description=(
            "No formal monitoring of jobs/integrations is performed to ensure timely detection and resolution of failures. "
            "There isn't a formalized ticketing or documentation process to track job failures, investigations, and resolutions. "
            "The process lacks defined timelines and escalation procedures for addressing failures, which could result in delayed identification and resolution of issues affecting financial data."
        ),
        recommendation=(
            "For the system(s) listed in column E, perform the following steps: "
            "1. Implement a formal procedure for monitoring the system integration job, including specific roles and responsibilities. "
            ...
            "5. Consider leveraging a monitoring tool that provides real-time visibility into the status of the system integration job. "
    ...
    GapExample(
        control_id="MO-01",
        gap_status="informal process",
        gap_title="Informal Scheduled Job / Integration Failure Monitoring",
        gap_description=(
            "Informal monitoring of jobs/integrations is performed to ensure timely detection and resolution of failures. "
            "There isn't a formalized ticketing or documentation process to track job failures, investigations, and resolutions. "
            "The process lacks defined timelines and escalation procedures for addressing failures, which could result in delayed identification and resolution of issues affecting financial data."
        ),
        recommendation=(
    ...
    GapExample(
        control_id="MO-02",
        gap_status="informal process",
        gap_title="Inadequate Data Backup Process",
        gap_description=(
    ...
    GapExample(
        control_id="MO-03",
        gap_status="informal process",
        gap_title="Inadequate Backup Restoration Testing",
        gap_description=(
            "While system backups are being performed for the system, the organization conducts limited and "
            "inconsistent testing of backup restoration capabilities. "
            "Restoration tests are performed on an ad-hoc basis rather than consistently following a periodic schedule (e.g., annually), "
            "with some periods potentially having no testing performed. "
    ...
    GapExample(
        control_id="ITELC-01",
        gap_status="gap",
        gap_title="Absence of Formal SOC Report Review Process",
        gap_description=(
    ...
    GapExample(
        control_id="ITELC-01",
        gap_status="informal process",
        gap_title="Informal SOC Report Review Process",
        gap_description=(
            "Management occasionally reviews SOC 1 Type II reports for the service organization, but lacks a formalized, "
